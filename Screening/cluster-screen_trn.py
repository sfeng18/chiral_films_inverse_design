...
if __name__ == "__main__":
    ...
    DataBasePath = './Data/uvdb_trn.msgz'  # Your Database
    ...
    ExpParas = np.array([(Film2num[f], *p) for f, p in zip(Films, ExpParas)])
    ...
    # plt.title("TRN_hist_max_ratio")
    plt.savefig('TRN_hist_max_ratio.png', dpi=300)
    # plt.savefig('TRN_hist_max_ratio.pdf', dpi=300)
    ...
    plt.xticks(fontsize=TickSize)
    plt.yticks(fontsize=TickSize)
    # plt.title("TRN_hist_avg_T0")
    plt.savefig('TRN_hist_avg_T0.png', dpi=300)
    # plt.savefig('TRN_hist_avg_T0.pdf', dpi=300)

    ### Hierarchy clustering

    scale_MaxRatio = np.max(MaxRatio)
    # scale_T0 = np.max(avg_T0)
    # scale_MaxRatio = np.quantile(MaxRatio, 0.5)
    # scale_T0 = np.quantile(avg_T0, 0.5)
    # scale_MaxRatio = 1
    scale_T0 = 100
    print(f'scale: MaxRatio: {scale_MaxRatio} T0: {scale_T0}')
    inputs = np.array([MaxRatio / scale_MaxRatio, avg_T0 / scale_T0]).T
    # inputs = np.array([Ratio_class,T0_class]).T
    ...
    with uv.timer('Run hierarchy'):
        ...
        plt.savefig("TRN_scipy_hierarchy.png", dpi=300)
        # plt.savefig("TRN_scipy_hierarchy.pdf", dpi=300)
        for i in set(f):
            print(f'Class {i}:')
        ...
        plt.savefig("TRN_scipy_hierarchy_colordots.png", dpi=300)
        # plt.savefig("TRN_scipy_hierarchy_colordots.pdf", dpi=300)
    ...
    bar_alpha = 0.7
    edge_width = 1
    col_names = ['Film', 'Thickness', 'Strain', 'DyeTime']
    cname2title = {
        'Film': 'Film material',
        'Color': 'Dye',
    ...
    my_axissizes = {
        'Film': (0.16, 0.16, 0.82, 0.80),
        'Color': (0.10, 0.16, 0.88, 0.80),
        'Thickness': (0.14, 0.16, 0.84, 0.80),
        'Thick': (0.14, 0.16, 0.84, 0.80),
        'Strain': (0.10, 0.16, 0.88, 0.80),
        'DyeTime': (0.13, 0.16, 0.85, 0.80),
        'NormStrain': (0.10, 0.16, 0.88, 0.80),
    }
    print('Probability:')
    for i, cname in enumerate(col_names):
        ...
        if cname == 'Film':
            print('%10s%6s%6s%6s%6s' % ('value', f'f={good_f}', f'f={bad_f}', 'rc=2', 'rc=0'))
            for k, p1, p2, p3, p4 in col_p:
                print(f'{Film_list[int(k)]:10s}{p1:6.2f}{p2:6.2f}{p3:6.2f}{p4:6.2f}')
        else:
        ...
        ax.set_xticks(x)
        if cname == 'Film':
            ax.set_xticklabels(Film_list, fontsize=TickSize)
        else:
        ...
        plt.savefig(f'TRN_Prob2_{cname}.png', dpi=120)
        # plt.savefig(f'TRN_Prob2_{cname}.pdf', dpi=120)
        plt.close()
    ...
    plt.suptitle('TRN_hierarchy')
    plt.savefig('TRN_points_hierarchy.png', dpi=120)
    # exit()

    ### Select good parameters (film, thickness, strain) for 2nd round screening
    ...
    for i in range(NFilms):
        ...
        max_strain = max(np.max(norm_strain[idx]), 1)
        norm_strain[idx] /= max_strain
        if Film_list[i] not in good_films:
            continue
        idx = np.where((ExpParas[:, 0] == i) & (ExpParas[:, 2] < 300))[0]
        fine_idx.extend(idx.tolist())
        all_film2fine[i] = len(all_film2fine)
        fine_films.append(Film_list[i])
    ...
    fine_paras[:, 0] = [all_film2fine[_] for _ in fine_paras[:, 0]]
    ...
    with uv.timer('Run hierarchy'):
        ...
        plt.savefig("TRN_fine_hierarchy.png", dpi=300)
        for i in set(f):
            print(f'Class {i}:')
            idx = np.where(np.array(f) == i)[0]
            my_ld = fine_ld[idx]
    ...
    print('%6s%6s%6s%8s%8s' % ('Color', 'Strain', 'DyeTime', 'NormStr', 'MaxRatio'))
    for i in set(fine_paras[:, 0].tolist()):
        idx = np.where(print_info[:, 0] == i)[0]
        for info in print_info[idx, :]:
            print('%6d%6d%6d%8.2f%8.2f' % (info[0], *info[2:]))
    ...
    interest_infos = np.hstack([fine_paras[:, [0, 1, 2]], (10 * fine_norm_strain).astype(int).reshape((-1, 1))])
    ...
    legend2 = ax2.legend(*scatter2.legend_elements(), title="Real classes")
    ax2.add_artist(legend2)
    plt.suptitle('TRN fine hierarchy')
    plt.savefig('TRN_fine_points_hierarchy.png', dpi=120)
    ...
    LegendSize = 16
    bar_alpha = 0.7
    col_names = ['Film', 'Thickness', 'Strain', 'NormStrain']
    print('Probability:')
    for i, cname in enumerate(col_names):
        print(cname)
        ...
        col_p.sort(key=lambda x: x[1], reverse=True)
        if cname == 'Film':
            print('%10s%6s%6s' % ('value', f'f={good_f}', f'f={bad_f}'))
            for k, p1, p2 in col_p:
                print(f'{fine_films[int(k)]:10s}{p1:6.2f}{p2:6.2f}')
        else:
        ...
        ax.set_xticks(x)
        if cname == 'Film':
            ax.set_xticklabels(fine_films, fontsize=TickSize)
        else:
            ax.set_xticklabels(labels, fontsize=TickSize)
        plt.yticks(fontsize=TickSize)
        plt.ylim(0, 100)
        ...
        ax.spines['left'].set_linewidth(edge_width)
        ax.spines['right'].set_linewidth(edge_width)
        plt.savefig(f'TRN_FineProb2_{cname}.png', dpi=120)
        # plt.savefig(f'TRN_FineProb2_{cname}.pdf', dpi=120)
        plt.close()
